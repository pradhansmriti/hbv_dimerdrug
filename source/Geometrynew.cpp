...
void Geometry::initialize(int Ntype0)
{
	...
	//gdrug_add_energy=new double*[Ntype];
	...
	for (int i = 0; i < Ntype; i++)
	{
		...
		//gdrug_add_energy[i]=new double[Ntype];
		for (int j = 0; j < Ntype; j++)
		{
			gb[i][j] = 0;
			gdrug[i][j] = 0;
			//gdrug_add_energy[i][j]=0;
		}
	}
...
{
	double e1 = find_dg(etypenew, etypenextofnew, 0,0);
	double e2 = find_dg(etypenextofnew, etypeprevifnew, 0,0);
	double e3 = find_dg(etypeprevifnew, etypenew, 0,0);
	//cout << "e1 is " << e1 << " e2 is " << e2 << " e3 is " <<e3 <<endl;
	return e1 + e2 + e3;
}
/*double Geometry::find_add_affinity(int type, int typenext, bool drug)
{
	double binding_e_drug=gdrug_add_energy[type][typenext]*drug;
	return binding_e_drug;
}*/
double Geometry::find_dg(int type, int typenext, bool drug, bool drug2)
{
...
/*        double bindg = gb[type][typenext];
	...
	double bindg = gb[type][typenext] + drug * (gdrug[type][typenext]-mudrug)+drug2*(0.2);
	/*if (((typenext == 0) || (typenext == 3)) && ((type == 0) || (type == 3))) {
...
double Geometry::compute_bind_energy()
{
	...
	for (vector<HE>::iterator it = he.begin(); it != he.end(); ++it)
	{
		if (it->nextid != -1)
		{
			...
			tot_bind += find_dg(it->type, he[nextid].type, he[nextid].din,it->din);
		}
	}
	return tot_bind;
}
